...
<div class="panel">
  <div class="panel__body">
    ...
    <div class="tablewrap">
      <table class="ui" aria-label="consultas vendedor">
        <thead>
          <tr>
            <th>Ticket</th>
            <th>Nombre</th>
        ...
        <tbody>
          <?php foreach ($data as $c): ?>
            <tr>
              ...
              <td>
                <div class="actions">
                  <a class="btn btn--sm" href="<?= htmlspecialchars(route('vendor.contacts.show', ['id' => $c->id_contacto]), ENT_QUOTES, 'UTF-8') ?>">Ver</a>
                </div>
              </td>
            </tr>
          <?php endforeach; ?>
    ...
    <?php if ($pages > 1): ?>
      <div class="pagination" style="margin-top:14px;">
        <?php if ($page > 1): ?>
          <a class="pagebtn" href="<?= htmlspecialchars(route('vendor.contacts.index') . '?' . http_build_query(['page' => $page - 1]), ENT_QUOTES, 'UTF-8') ?>">‹</a>
        <?php endif; ?>

        <?php for ($p = 1; $p <= $pages; $p++): ?>
          <a class="pagebtn <?= ($p === $page) ? 'pagebtn--active' : '' ?>" href="<?= htmlspecialchars(route('vendor.contacts.index') . '?' . http_build_query(['page' => $p]), ENT_QUOTES, 'UTF-8') ?>"><?= $p ?></a>
        <?php endfor; ?>
